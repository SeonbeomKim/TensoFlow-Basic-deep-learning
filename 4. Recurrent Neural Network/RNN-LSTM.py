...
def test(data):
	...
	for i in range( int(math.ceil(len(data)/batch_size)) ):
		...
		check = sess.run(correct_check, {X:input_, Y:target_})
		correct += check
...
with tf.device('/gpu:0'):
	...
	X_reshape = tf.reshape(X, (-1, time_, vector_len_each_time_))

	cell = tf.nn.rnn_cell.LSTMCell(cell_num, state_is_tuple=True) #GRUCell, RNNCell
	val, state = tf.nn.dynamic_rnn(cell, X_reshape, dtype=tf.float32)
	...
	W = tf.get_variable('w', shape = [cell_num, 10], initializer=tf.contrib.layers.xavier_initializer())
...
batch = train_set[:3]
input_ = batch[:, :784]
input_ = np.reshape(input_, (-1, 28, 28))
target_ = batch[:, 784:]
print(sess.run(val, {X:input_}))
print(sess.run(last_val, {X:input_}))
'''
